...
func (a *Adapter) Run(ctx context.Context, opts spec.RunOptions) (spec.RunResult, error) {
	...
	stdout, stderr, code, err := run(ctx, "podman", args, false)
	if opts.Detach {
...
func (a *Adapter) Logs(ctx context.Context, containerID string, follow bool) (string, error) {
	...
	stdout, stderr, _, err := run(ctx, "podman", args, false)
	if err != nil {
...
func (a *Adapter) Inspect(ctx context.Context, containerID string) (string, error) {
	stdout, stderr, _, err := run(ctx, "podman", []string{"inspect", containerID}, false)
	if err != nil {
		return stdout + stderr, err
	}
	return stdout, nil
...
func (a *Adapter) Remove(ctx context.Context, containerID string) error {
	_, _, _, err := run(ctx, "podman", []string{"rm", "-f", containerID}, false)
	return err
}
...
func run(ctx context.Context, bin string, args []string, stdin bool) (string, string, int, error) {
